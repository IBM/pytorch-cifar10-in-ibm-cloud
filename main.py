...
parser.add_argument('-l', action='store_true',
                    help='lists all available models')
parser.add_argument('--lr', default=0.1, type=float, help='learning rate')
parser.add_argument('--resume', '-r', action='store_true',
                    help='resume from checkpoint')
parser.add_argument('--cifar_path',
                    default='./cifar10',
                    type=str, help='path to cifar tar.gz')
...
if args.l:
    for name, obj in inspect.getmembers(models):  # what do I do here?
        if inspect.isclass(obj) and not name == 'VGG':
            print(name, ' (', obj, ')')
        if inspect.isfunction(obj) and name[0].isupper():
            print(name, ' (', obj, ')')
        if name == 'VGG':
            for vgg in models.vgg_cfg.keys():
                print(vgg, ' ( config in ', obj, ')')
    exit(0)

# else:

input_data_folder = args.cifar_path
model_path = args.checkpoint_path
model_file = lambda x: os.path.join(model_path, "chkpt_" + str(x) + ".pt")
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        #     os.mkdir('checkpoint')
        torch.save(state, model_file(str(epoch) + '_' + str(acc)))
        best_acc = acc
